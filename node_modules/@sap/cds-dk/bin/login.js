...
module.exports = Object.assign(login, {
    options: ['--subdomain', '--directory', '--passcode', '--user', '--client'],
    shortcuts: ['-s', '-d', '-p', '-u', '-c'],
    flags: ['--plain'],
    get help() {
...
# OPTIONS
    ...
    *-s* | *--subdomain* <tenant-subdomain>
        ...
        The subdomain to log in to. By default, it's determined from project settings.
        If no such setting is saved, the CF client is invoked to determine the respective subdomain.

    *-d* | *--directory* <extension-project-directory>

        The extension-project folder (relevant for loading and saving settings).
        If omitted, either an existing subdirectory named as the subdomain or the current
    ...
    *-u* | *--user* <name>[:[<password>]]
        ...
        When typing the colon but omitting <password>, the password is assumed empty.
        When omitting both, password will be queried interactively.

        For security reasons, username will only be saved for app URLs that look like localhost.
        The same is true for an empty password. Non-empty passwords will never be saved.
        If saved, a username will be associated with the project folder, not with the (potentially ambiguous) URL.
        
        Note: Productive secrets should not be specified on a command line, since this might expose them to
        unauthorized users on the same machine!

    *-c* | *--client* <clientid>[:<clientsecret>]

        Client ID and optionally Client Secret for authentication. If omitted, Client Secret will be
        queried interactively.
        
        Neither Client ID nor Client Secret will be saved, only the resulting token.
    ...
    *--plain*
        ...
        least applications running as the current user can read the plain-text file,
        whereas the keyring (depending on the platform) can be locked and unlocked on owner
        request and may hide data saved by this command from other applications you run.

        Be aware that on SAP Business Application Studio, --plain is enforced since keyring
        functionality is not supported. However, the underlying storage is encrypted.
