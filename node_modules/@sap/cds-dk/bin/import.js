...
# OPTIONS
    ...
    *-as | --as <output_file_format>*
        Converts the input file into one of these supported formats "cds", "csn" or "json".

    *-f | --force*
        Forcefully overwrites the contents in the CDS file and updates the checksum

    *--from*
        Imports the model from the one of the given options "edmx" or "openapi".
...
async function _import(file, options = {}) {
    ...
    const [srcFilePath, destFilePath] = await common.preProcess(file, options, cwd);
    ...
    options.inputFileVersion = apiOptions.inputFileVersion;

    // postprocess
    return await common.postProcess(destFilePath, options, csn, cwd);
}
