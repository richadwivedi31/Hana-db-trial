...
# SYNOPSIS
    ...
    *cds push* [ <extension-project-directory> | <extension-archive> | <extension-url> ]
    ...
    You may specify a project directory or a custom extension archive, either as a
    local file or as a URL to download it from.
    A download URL may contain Basic-Auth credentials as in 'https://user:pass@host'.
    
    By default, the current working directory is used as project directory.
    
    Unless a custom archive is given, the extension project will be automatically built
    in order to update the generated extension archive. This ensures the most recent
    extension model is pushed.
...
# OPTIONS
    ...
    *--to* <url>
        ...
        Specify the URL of the extensible SaaS app. Determined from the current user's active
...
async function push([projectFolder], options = {}) {
    ...
    let extArchive;
    if (projectFolder && !isDir(projectFolder)) {
    ...
    await require('../lib/client/push').run(...args);
}
