...
const { add, promote } = require('./extensibility/add')
...
const { addCodeAnnotations } = require('./extensibility/code-extensibility/annotations')
...
module.exports = class ExtensibilityService extends cds.ApplicationService {
  ...
  async init() {
    ...
    if (cds.env.requires.extensibility?.code && !_in_prod && !cds.requires.multitenancy) {
      const findings = await addCodeAnnotations(true)
