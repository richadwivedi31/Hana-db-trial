...
module.exports = class AllowlistChecker {
  ...
  check(reflectedExtensionCsn, fullCsn, codeExt, compileDir, mtxConfig) {
    ...
    if (codeExt.extensions && allowList) {
      for (const extension of codeExt.extensions) {
        this._checkExtensionCode(extension, fullCsn, allowList, warnings)
      }
    }
  ...
  _checkEntity(extension, extCsn, fullCsn, compileDir, allowlist, warnings) {
  ...
  _checkActionFunction(actionKind, checkResult, name, allowlist, warnings, kind) {
    if (actionKind === 'action' && !checkResult.code.includes('action')) {
      this._addCodeWarnings(name, warnings, 'action', allowlist.getList(kind), kind)
    }
    if (actionKind === 'function' && !checkResult.code.includes('function')) {
      this._addCodeWarnings(name, warnings, 'function', allowlist.getList(kind), kind)
    }
  }
  
  _checkExtensionCode(extension, fullCsn, allowlist, warnings) {
    const extendedEntity = extension.annotate
    if (extendedEntity && fullCsn?.definitions) {      
      const kind = this._getExtendedKind(fullCsn, extendedEntity)
      if (kind === 'entity') {
        const checkResult = allowlist.isAllowed(kind, extendedEntity)
        if (!checkResult) {
          return this._addElementCodeWarnings(extension, warnings, allowlist.getList(kind), kind)
        }
        const codeExt = extension['@extension.code']
        if (codeExt) {
          codeExt.forEach(element => {
            const operation = element.before || element.after
            if (!checkResult.code.includes(operation)) {
              this._addCodeWarnings(extension.annotate, warnings, operation, allowlist.getList(kind), kind)
            }
          })
        } 
        if (extension.actions && Object.keys(extension.actions).length) {
          const actionKind = fullCsn.definitions[extendedEntity].actions[Object.keys(extension.actions)[0]].kind
          this._checkActionFunction(actionKind, checkResult, extension.annotate, allowlist, warnings, kind)
        }
        ...
        this._checkActionFunction(kind, checkResult, serviceName, allowlist, warnings, 'service')
      }
    }
  }

  _getExtendedKind(fullCsn, extendedEntity) {
    const elementFromBase = fullCsn.definitions[extendedEntity]
    return elementFromBase ? elementFromBase.kind : null
  }

  _addElementWarnings(extension, warnings, compileDir, allowlist, kind) {
  ...
  _addElementCodeWarnings(extension, warnings, allowlist, kind) {
    warnings.push(
      this._createAllowlistWarning(
        extension.annotate,
        'code',
  ...
  _addCodeWarnings(name, warnings, operation, allowlist, kind) {
    warnings.push(
  ...
  _addColumnWarnings(extension, warnings, compileDir, allowlist, kind) {
  ...
  _addEntityLimitWarnings(extension, warnings, compileDir, allowlist, kind) {
  ...
  _addServiceLimitWarnings(foundServiceExt, allowlist, compileDir, warnings) {
    if (!allowlist) {
      return
    }
  ...
  _checkService(reflectedExtensionCsn, fullCsn, element, parent, extension, allowlist, warnings, foundExt) {
  ...
  _createLimitWarning(entityName, element, compileDir, limit, label) {
    let message = `Extension limit of ${limit} for ${label} '${entityName}' has been exceeded`
    return { message, element }
  }
