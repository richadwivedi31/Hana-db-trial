...
async function create(tenant, parameters) {
  LOG.info(`creating HDI container for tenant ${tenant}`)
  ...
  try {
    _instance = await api.post('service_instances', {
      name, service_plan_id, parameters: parameters?.provisioning_parameters,
  ...
  const { data } = await api.post('service_bindings', {
    name: tenant + `-${uuid()}`, service_instance_id, binding_parameters: parameters?.binding_parameters,
