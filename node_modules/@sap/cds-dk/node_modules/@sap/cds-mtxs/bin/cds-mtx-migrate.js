...
async function run(param, options) {
    ...
    const { migrate,  cleanupMetatenants, _hasMtEnv } = require('../lib/migration/migration')
    const { dry, cleanup } = options
    ...
    async function cds_mtx_migrate(tenants, options) {
        ...
        if (cleanup) {
            ...
            for (t of deletedTenants.connectedTenants) await cds.db.disconnect(t)
            await cds.db.disconnect()

            migratedTenants.logResult(dry)
