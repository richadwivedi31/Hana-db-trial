...
const cdsCompiler = require('../compile').compile.to;
...
async function _getODataVersion(edmx) {
    ...
    jsonObj = await generateEDMX2JSON(edmx);
    try {
...
async function preProcess(file, options, cwd) {
    ...
    const destFilePath = path.join(external, path.basename(srcFilePath));

    if (!options.dry && !options.no_copy)
        srcFilePath = await _copyToSrvExternal(srcFilePath, destFilePath, cwd);
...
async function _updatePackageJson(dest, service, version, cwd) {
...
async function postProcess(filePath, options, csn, cwd) {
    const service = _getServiceName(csn);
    ...
    const written = options.dry || await _write(result, service, cwd);
    const registered = options.dry || options.no_save || await _updatePackageJson(result[1].replace(/\.[^.]+$/, ''), service, version, cwd);
