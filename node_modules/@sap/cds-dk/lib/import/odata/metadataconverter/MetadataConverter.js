...
class Expression {
    ...
    includeExternalAnnotations(element, target) {
        ...
        for (let i = 0; i < nonStdAttributes.length; i++) {
            ...
            if ((MetadataConverter.custom_namespace.includes(prefix) || MetadataConverter.custom_namespace.includes(true)) && attrName !== undefined)
                ...
                attribute = "$" + prefix;
            if (attribute !== undefined && attribute.startsWith('$') === false) {
                Object.entries(target).forEach(a => {
                    if (Object.keys(a[1]).length === 0 && typeof (a[1]) != 'boolean') {
                        a[1][attribute] = element.attributes[attr];
                        check = 1;
                    }
                });
                if (check == 0)
    ...
    getNonStdAttributes(attributes) {
        const attributeList = [];
        const standardArrtibutes = ['MaxLength', 'Precision', 'Scale', 'Nullable', 'DefaultValue',
            'Name', 'Type', 'HasStream', 'IsFlags', 'Alias', 'Namespace', 'BaseType', 'Partner',
            'IsComposable', 'ContainsTarget', 'IsBound', 'EntitySet', 'EntitySetPath', 'EntityType'];
