...
module.exports.csdl2openapi = function (
    ...
    const deepUpdate = entityContainer ? entityContainer[voc.Capabilities.DeepUpdateSupport] && entityContainer[voc.Capabilities.DeepUpdateSupport].Supported : {};
    ...
    function operationCreate(pathItem, element, name, sourceName, targetName, target, level, restrictions) {
        ...
        if (insertRestrictions.Insertable !== false) {
            ...
            const type = modelElement(element.$Type);
            pathItem.post = {
                summary: insertRestrictions.Description || operationSummary('Create', name, sourceName, level, true, true),
                tags: [sourceName],
                requestBody: {
                ...
                responses: response(201, 'Created ' + lname, { $Type: element.$Type }, insertRestrictions.ErrorResponses),
            };
    ...
    function operationRead(pathItem, element, name, sourceName, targetName, target, level, restrictions, byKey, nonExpandable) {
        ...
        if (readable) {
            ...
            const operation = {
                summary: descriptions.Description || operationSummary('Retrieve', name, sourceName, level, element.$Collection, byKey),
                tags: [sourceName],
                parameters: [],
                responses: response(200, 'Retrieved ' + (byKey ? pluralize.singular(lname) : lname), { $Type: element.$Type, $Collection: collection },
                byKey ? readByKeyRestrictions?.ErrorResponses : readRestrictions?.ErrorResponses)
            };
...
see [Expand](http://docs.oasis-open.org/odata/odata/v4.01/odata-v4.01-part1-protocol.html#sec_SystemQueryOptionexpand)',
    ...
    function operationUpdate(pathItem, element, name, sourceName, target, level, restrictions, byKey) {
        ...
        if (updateRestrictions.Updatable !== false) {
            const type = modelElement(element.$Type);
            const operation = {
                summary: updateRestrictions.Description || operationSummary('Change', name, sourceName, level, element.$Collection, byKey),
                tags: [sourceName],
                requestBody: {
                ...
                responses: response(204, "Success", undefined, updateRestrictions.ErrorResponses),
            };
    ...
    function operationDelete(pathItem, element, name, sourceName, target, level, restrictions, byKey) {
        ...
        if (deleteRestrictions.Deletable !== false) {
            pathItem.delete = {
                summary: deleteRestrictions.Description || operationSummary('Delete', name, sourceName, level, element.$Collection, byKey),
                tags: [sourceName],
                responses: response(204, "Success", undefined, deleteRestrictions.ErrorResponses),
            };
    ...
    */
    ...
    function schemaForEnumerationType(schemas, qualifier, name, type) {
        ...
        const description = type[voc.Core.Description];
    ...
    function schemasForStructuredType(schemas, qualifier, name, type, suffix) {
        ...
        Object.keys(properties).forEach(iName => {
            ...
            if (property.$Kind == 'NavigationProperty') {
                ...
                if (property[voc.Core.Permissions] != "Read" && !property[voc.Core.Computed] && property.$ContainsTarget) {
    ...
    function getSchema(element, suffix = '', forParameter = false, forFunction = false) {
        ...
        if (!forParameter && element[voc.Core.Description]) {
            if (s.$ref) s = { allOf: [s] };
            s.description = element[voc.Core.Description];
        }
