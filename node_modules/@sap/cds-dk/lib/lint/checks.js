...
module.exports = {
  ...
  /**
   * Checks if VS Code is installed. If not, suggests to install it
   * @param {*} inputPath CDS project root
   * @returns boolean whether VS Code is installed
   */
  async hasVscode(inputPath, logger = console) {
    try {
      ...
      logger.log(`Consider installing VS Code: ${URLS.VSCODE}`);
      return false;
    }
    return true;
  },
  ...
  /**
   * Checks whether VS Code extension <extension> is installed
   * @param {*} extension VS Code extension
   * @param {*} inputPath CDS roject root
   * @returns boolean whether VS Code extension is installed
   */
  async hasVscodeExtension(extension, inputPath, logger = console) {
    try {
    ...
    if (this.linterExtensions && this.linterExtensions.includes(extension)) {
      return true;
    } else {
      logger.log(
