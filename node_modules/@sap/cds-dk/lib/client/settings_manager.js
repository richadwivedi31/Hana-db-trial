...
function logConfigPaths() {
    log.debug(`Settings are stored in ${CONFIG.paths.settings}`);
    log.debug(`Authentication data is stored in ${CONFIG.paths.auth} (${TOKEN_STORAGE_DESC.plain}) or ${CONFIG.keyringDesignation} (${TOKEN_STORAGE_DESC.keyring})`);
}
...
class SettingsManager {
    ...
    static async saveSettings(params) {
        log.debug(`Saving settings for project ${params.get('projectFolder')}`);
        ...
        if (params.has('username')) {
            ...
            if (localhostRegex.test(params.get('appUrl'))) {
                ...
                log.debug('Not saving username because app is not recognized to run on localhost.');
    ...
    static async loadAndMergeSettings(params, logout = false) {
        ...
        log.debug(`Loaded project settings for ${params.get('projectFolder')}: ${loadedSettings.format()}`);
        ...
        log.debug(`Effective project settings: ${params.format()}`);
    }
        ...
        async function addAppUrlAndSubdomain() {
            ...
            if (params.has('appUrl')) {
                ...
                if (!schemaRegex.test(appUrl)) {
                    ...
                    log.debug(`Added schema to app URL: ${params.get('appUrl')}`);
                }
            }
            ...
            if (!logout) {
                // Get appUrl from CF
                if (!params.has('appUrl')) {
                    log.debug('App URL not given');
                    await getAppUrlAndSubdomainFromSuggestions(params);
                }

                // Get subdomain from CF
                if (!params.has('subdomain') && needsSubdomain()) {
                    log.debug('Subdomain not given');
            ...
            // Check required param: subdomain
            if (!params.has('subdomain') && needsSubdomain()) {
                const subdomain = extractSubdomain(params.get('appUrl'));
                if (!subdomain) {
                    throw new CliError('Subdomain not given');
                }
                params.set('subdomain', subdomain);
                console.log('Subdomain determined from app URL:', subdomain);
            }
        }
    ...
    static async addAuth(params, logout) {
        ...
        function setRelevantAuth(auth) {
            ...
            if (auth.keyring.has('tokenStorage') && auth.plain.has('tokenStorage')) {
                ...
                log.debug('WARNING: authentication data found in both kinds of storage. ' +
                ...
                log.debug(`Using authentication data from ${TOKEN_STORAGE_DESC[other(storage)]}; will be migrated to other storage on save.`);
                ...
                log.debug(`Using authentication data from ${TOKEN_STORAGE_DESC[storage]}.`);
        ...
        if (params.has('username')) {
            ...
            log.debug('Ignoring any saved authentication data because username is given');
            return;
        } else {
            params.delete('isEmptyPassword');
        }
        ...
        if (!logout && !params.has('token') && !params.has('passcode') && !params.has('clientsecret')) {
            await addPasscode();
        }
    }
    ...
    static async deleteToken(params, fromOtherStorage = false) {
        ...
        log.debug(`Deleting authentication data${fromOtherStorage ? ' from other storage' : ''} for ${target}`);
    ...
    static async _saveToFile(projectFolder, paramValues) {
        ...
        if (paramValues !== null) {
            ...
            log.debug(`Saved project settings: ${JSON.stringify(paramValues)}`);
        } else {
    ...
    static async _saveAllSettingsToFile(paramValuesByFolder) {
        log.debug(`Saving all settings to ${CONFIG.paths.settings}`);
    ...
    static async _loadFromFile(projectFolder) {
        ...
        if (!settingsByFolder[projectFolder]) {
            log.debug(`No settings found for project ${projectFolder}`);
    ...
    static async _loadAuthFromFile(appUrl, subdomain) {
        ...
        if (!fs.existsSync(CONFIG.paths.auth)) {
            log.debug('Authentication-data file not found');
            return emptyData(all);
        }
        ...
        try {
            ...
            log.debug('Empty authentication-data file');
            return emptyData(all);
        }
        ...
        if (!allAuth[appUrl]) {
            log.debug(`File contains no authentication data for app URL ${appUrl}`);
            return new ParamCollection();
        }
        ...
        if (!authForSubdomain) {
            log.debug(`File contains no authentication data for subdomain ${subdomain} (app URL: ${appUrl})`);
            return new ParamCollection();
        }
    ...
    static async _loadAuthFromKeyring(appUrl, subdomain) {
        ...
        if (!authString) {
            log.debug(`${capitalize(TOKEN_STORAGE_DESC.keyring)} contains no authentication data for URL ${appUrl} and subdomain '${subdomain}'`);
            return new ParamCollection();
        }
