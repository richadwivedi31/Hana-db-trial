...
const { escapeRegex } = require('./helper/string_helper');
...
class Param {
    ...
    validate(val) {
        ...
        if (typeof val !== expectedType) {
            throw new CliError(`Failed to set '${(this.name)}' = ${JSON.stringify(val)}: expected value of type ${expectedType}`);
        }
        if (this.allowedValues && !(this.allowedValues).includes(val)) {
            throw new CliError(`Failed to set '${(this.name)}' = ${JSON.stringify(val)}: expected one of ${JSON.stringify(this.allowedValues)}`);
        }
    }
        ...
        return this.internal
            ...
            : this.obfuscate === ObfuscationLevel.none && (!this.abbreviate || process.env.DEBUG)
                ...
                : this.obfuscate === ObfuscationLevel.partial && process.env.DEBUG && typeof this.val === 'string'
                    ? this.val.substr(0, DISCLOSURE_LENGTH) + '...' + this.val.substr(-DISCLOSURE_LENGTH)
                    : '...';
    }
...
class BasicParams {
    constructor() {
        ...
        // INTERNAL PARAMS (HIDDEN FROM USER, COMPUTED)
...
class ParamCollection {
    ...
    /**
     * Whether this includes the given param.
     * @param name the param in question
     */
    has(name) {
    ...
    /**
     * Return the `Param` object for the given param.
     * @param name the param in question
     */
    getParam(name) {
    ...
    set(name, val) {
        ...
        if (mainName !== name) {
            log.debug(`Migrated setting '${name}' to new key '${mainName}'`);
        }
    }
