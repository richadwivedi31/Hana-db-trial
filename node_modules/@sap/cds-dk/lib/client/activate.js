...
module.exports = class Activate extends BaseCommand {
    ...
    static async run(injection, paramValues) {
        ...
        if (!await this.checkProject(params.get('projectFolder'))) {
            throw new CliError(params.get('projectFolder') + ' is not a valid extension project');
        }
    ...
    static async collectExtensionFiles(cds, projectFolder) {
        ...
        const filteredSources = sources && sources.filter((entry) => {
            if (! entry.startsWith(projectFolder)) {
                ...
                log.debug(`Source file outside project folder will not be uploaded: ${entry}`);
                return false;
            }
    ...
    static async runActivate(cds, params) {
        ...
        .then(allFiles => {
            ...
            activationUrl = this.getMtxApiUrl(params.get('appUrl'), urlSuffix);
        ...
        .then(response => {
            ...
            if (jobId === undefined) {
                throw new CliError('Activation failed: did not receive job ID');
            }

            if (!statusPath) {
                throw new CliError(`Job ${jobId} failed: did not receive job-status path`);
            }

            log.debug(`Job ID: ${jobId}`);

            return new Promise((resolve, reject) => {
                const statusUrl = this.concatUrls(params.get('appUrl'), statusPath);
                const finisher = {
                    resolve,
                    reject,
                    finished: false,
    ...
    static async runCustomFileUpload(params) {
        ...
        const customFileUrl = this.getMtxApiUrl(params.get('appUrl'), this.UPDATE_CUSTOM_FILE_URL_SUFFIX);
        const requestOptions = Object.assign({
            headers: {},
        }, params.get('reqAuth'));
        requestOptions.headers['Content-Type'] = 'application/json';
