...
async function getAppFromSuggestions() {
    ...
    if (apps.length === 0) {
        log.debug('Failed to obtain list of apps from CF');
        return undefined;
    }
...
async function getApps(nameRegex = /(?<!-db)$/) {
    log.debug('Getting running apps from CF');
    ...
    function errorResult(message) {
        log.debug(`Failed to get apps: ${message}`);
        return [];
    }
}

async function getSubdomain(appName) {
    log.debug('Getting subdomain from CF env');
    ...
    return new Promise(resolve => {
        runCf(['env', appName], function(error, stdout) {
            ...
            log.debug(`Subdomain determined from CF app environment: ${value}`);
            return resolve(value);
        });
    });

    function errorResult(message) {
        log.debug(`Failed to get subdomain: ${message}`);
        return undefined;
    }
}
...
async function getAppName(appUrl) {
    ...
    log.debug('Getting app name from CF apps');
    const apps = await getApps();
    const app = apps.find(app => equalIgnoringTls(app.url, appUrl));
    if (!app) {
        log.debug(`Failed to find app with URL ${appUrl}.  Running apps: ${apps.map(app => app.name)}`);
        return;
    }
    log.debug(`App name determined from CF: ${app.name}`);
    return app.name;
}
