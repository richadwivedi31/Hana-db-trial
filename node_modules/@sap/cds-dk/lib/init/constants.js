const os = require('os');

const MAVEN_ARCHETYPE_VERSION = '1.31.1';

const constants = {
    IS_WIN: (os.platform() === 'win32'),

    COMMAND_INIT: 'init',
    COMMAND_ADD: 'add',

    MAVEN_ARCHETYPE_VERSION,

    OPTION_NODEJS: 'nodejs',
    OPTION_JAVA: 'java',
    OPTION_HANA: 'hana',
    OPTION_XSUAA: 'xsuaa',
    OPTION_DESTINATIONS: 'destinations',
    OPTION_MTA: 'mta',
    OPTION_MTX: 'mtx',
    OPTION_MULTITENANCY: 'multitenancy',
    OPTION_TOGGLES: 'toggles',
    OPTION_EXTENSIBILITY: 'extensibility',
    OPTION_AUDITLOG: 'auditlog',
    OPTION_ENTERPRISE_MESSAGING: 'enterprise-messaging',
    OPTION_APPROUTER: 'approuter',
    OPTION_KIBANA_LOGGING: 'kibana-logging',
    OPTION_CF_MANIFEST: 'cf-manifest',
    OPTION_PIPELINE: 'pipeline',
    OPTION_SAMPLES: 'samples',
    OPTION_DATA: 'data',
    OPTION_NOTEBOOK: 'notebook',
    OPTION_LINT: 'lint',
    OPTION_LINT_GLOBAL: 'lint:global',
    OPTION_LINT_DEV: 'lint:dev',
    OPTION_JAVA_PACKAGE: 'java:package',
    OPTION_JAVA_MVN: 'java:mvn',
    OPTION_HELM: 'helm',

    // node enum
    PROJECT_TYPE: Object.freeze({ nodejs: 1, java: 2, unknown: 3 }),

    PYTHON_VENV_NAME: ".cap-jupyter-nb",

    REGEX_JAVA_PACKAGE: /^[a-z_][a-z0-9_]*(\.[a-z_][a-z0-9_]*)*$/g,
    REGEX_PROJECT_NAME: /[^0-9a-zA-Z_-]/g,

    URLS: Object.freeze({
        PIPELINE_HELP: 'https://sap.github.io/jenkins-library/',
        SAMPLES: 'https://github.com/SAP-samples/cloud-cap-samples',
        CAPIRE: 'https://cap.cloud.sap',
        JUPYTERLAB: 'https://jupyterlab.readthedocs.io/en/stable/getting_started/installation.html',
        MAVEN_ARCHETYPE_HELP: `https://cap.cloud.sap/docs/java/development/#the-maven-archetype`,
        MAVEN_INSTALL_HELP: `https://maven.apache.org/install.html`,
        PYTHON: 'https://www.python.org/downloads/',
        PYTHON_PIP: 'https://packaging.python.org/en/latest/guides/',
        VSCODE: 'https://code.visualstudio.com/',
        VSCODE_NBVIEWER: 'https://marketplace.visualstudio.com/items?itemName=ms-python.python',
        VSCODE_ESLINT: 'https://marketplace.visualstudio.com/items?itemName=dbaeumer.vscode-eslint'
    }),

    PROJECT_FILES: ['package.json', 'pom.xml', '.cdsrc.json'],

    KIND_SQL: 'sql',
    KIND_HANA: 'hana',
    KIND_HANA_CLOUD: 'hana-cloud',
    KIND_HANA_MT: 'hana-mt'
}

constants.OPTIONS_PRIORITY = [
    constants.OPTION_JAVA,
    constants.OPTION_NODEJS,
    constants.OPTION_PIPELINE,
    constants.OPTION_MTA,
    constants.OPTION_CF_MANIFEST,
    constants.OPTION_MTX,
    constants.OPTION_MULTITENANCY,
    constants.OPTION_TOGGLES,
    constants.OPTION_EXTENSIBILITY,
    constants.OPTION_XSUAA,
    constants.OPTION_HANA,
    constants.OPTION_AUDITLOG,
    constants.OPTION_ENTERPRISE_MESSAGING,
    constants.OPTION_APPROUTER,
    constants.OPTION_DESTINATIONS,
    constants.OPTION_SAMPLES,
    constants.OPTION_DATA,
    constants.OPTION_KIBANA_LOGGING,
    constants.OPTION_NOTEBOOK,
    constants.OPTION_LINT,
    constants.OPTION_LINT_CUSTOM,
    constants.OPTION_LINT_EXAMPLE,
    constants.OPTION_HELM
];

module.exports = constants;
