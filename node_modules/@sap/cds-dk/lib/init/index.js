...
const { OPTIONS_PRIORITY, COMMAND_ADD, COMMAND_INIT,
    // eslint-disable-next-line no-unused-vars
    OPTION_XSUAA, OPTION_AUDITLOG, OPTION_HANA, OPTION_MTA, OPTION_MTX, OPTION_APPROUTER, OPTION_KIBANA_LOGGING,
    OPTION_NODEJS, OPTION_JAVA, OPTION_HELM,
    OPTION_PIPELINE, OPTION_SAMPLES, OPTION_CF_MANIFEST, PROJECT_FILES, REGEX_PROJECT_NAME } = require('./constants');
...
module.exports = class CDSGenerator {
        ...
        this.uiConfig = [
            ...
            },
            {
                ...
                choices: [
                    ...
                    // { name: `CDS Linter for custom rules`, value: OPTION_LINT },
                    // { name: `CDS Linter for custom rules with sample`, value: OPTION_LINT_DEV }
                ],
                multiple: true,
                target: 'add',
                guiOptions: {
    ...
    async addCmd(templates, options = {}) {
        ...
        if (!templates) throw 'You must specify a facet to add to the project.'
    ...
    _greetings(cmd) {
        switch (cmd) {
            case COMMAND_INIT: {
                ...
                LOG.info(`Creating new cap project in ${folderName}`);
                break;
            }
            case COMMAND_ADD:
                LOG.info(`Adding feature(s) to project in current folder`);
                break;
        }
    ...
    async _fillTemplateList() {
        ...
        for (const option of this.options.add) {
            if (!templates.has(option)) {
                ...
                if (await template.canRun()) {
                    DEBUG && DEBUG(`Adding feature '${template.name}'`);
        ...
        DEBUG && DEBUG(`Features: ${this.templateList.map(({name}) => name).join(', ')}`);
    }
    ...
    _cleanupOptions() {
        ...
        const trimmedTokens = tokens.map((token) => {
            ...
            if (tokenSplit.length > 1) {
                if (this.options[tokenSplit[0]]) {
                    this.options[tokenSplit[0]].add(token);
                } else {
                    this.options[tokenSplit[0]] = new Set([token]);
                }
            }
            return tokenSplit[0];
        }).filter((token) => {
        ...
        if (this.options._cmd === COMMAND_INIT && !this.options.add.has(OPTION_JAVA)) {
