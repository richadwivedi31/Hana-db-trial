...
class CommandUtil {
     ...
     * @returns Promise
     */
    async spawnCommand(cmd, args, options = {}, collectResult = false) {
        args = this._sanitizeArgs(args);
        return new Promise((resolve, reject) => {
            DEBUG && console.debug(`[spawn] ${cmd} ${args.join(' ')}`);
            ...
            const stdLogBuffer = new LogBuffer((chunk) => console.log(chunk), collectResult);
            ...
            const errLogBuffer = new LogBuffer((chunk) => console.error(chunk), (IS_WIN || collectResult));
